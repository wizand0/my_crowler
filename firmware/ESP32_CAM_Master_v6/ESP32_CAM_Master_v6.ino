...
bool initCamera() {
  ...
  s->set_hmirror(s, 0); // Отзеркаливание (чтобы право и лево не перепутались)
  // -----------------------------------------------
...
bool startRecordingSession() {
  ...
  while (true) {
    ...
    if (!SD_MMC.exists(path.c_str())) {
      ...
      // Сразу создаём CSV-файл с заголовком — чтобы даже без LOG-событий
      // было видно, что запись инициализирована корректно
      ...
      if (f) {
        ...
        f.println(String(millis()) + ",BOOT,0,0,0,0,0,0");
        f.close();
      }
      return true;
    }
...
// ============================================================
//  ЗАДАЧА АВТОНОМНОЙ ЗАПИСИ КАДРОВ
//  Работает независимо от того, смотрит ли кто-то MJPEG-поток.
//  Гарантирует запись на SD даже без подключённого клиента.
// ============================================================
void recorderTask(void* arg) {
  for (;;) {
    // Пишем только если никто сейчас не читает MJPEG-поток
    // (иначе запись делает handleStream — избегаем гонки за fb)
    bool streamActive = (activeStreams > 0);

    if (isRecording && sdOK && !streamActive) {
      unsigned long now = millis();
      if (now - lastFrameSave >= FRAME_SAVE_MS) {
...
static esp_err_t handleRec(httpd_req_t* req) {
  ...
  if (qLen > 0 && qLen < sizeof(buf)) {
    ...
    if (httpd_query_key_value(buf, "on", val, sizeof(val)) == ESP_OK) {
      bool on = (val[0] == '1');
      if (on && !isRecording) {
        // Возобновить запись — оператор разблокирует
        isRecording = startRecordingSession();
        recLocked = false;  // разрешили ручное управление
      } else if (!on && isRecording) {
        // Разрешаем остановку ТОЛЬКО после того, как автозапись
        // была явно разблокирована оператором (recLocked=false).
        // При первом включении recLocked=true — запись нельзя остановить,
        // пока оператор сначала не нажмёт REC чтобы разблокировать.
        if (!recLocked) {
          isRecording = false;
          sessionDir  = "";
          frameNum    = 0;
        ...
        // Если recLocked=true — игнорируем попытку остановить
      }
    }
  }
  httpd_resp_set_type(req, "text/plain");
...
static esp_err_t handleStream(httpd_req_t* req) {
  ...
  while (true) {
    ...
    // Сохраняем кадр параллельно со стримингом — единая точка доступа к fb
    if (isRecording && sdOK && millis() - lastFrameSave >= FRAME_SAVE_MS) {
      saveFrame(fb);
  ...
  activeStreams--;   // НОВОЕ: поток закрылся
...
void setup() {
  ...
  // ── БЕЗУСЛОВНЫЙ АВТОСТАРТ ЗАПИСИ ─────────────────────────
  // Запись начинается ВСЕГДА при включении, независимо от клиента.
  // Оператор может её остановить только если recLocked=false.
  if (sdOK) {
    isRecording = startRecordingSession();
    if (isRecording) {
  ...
  // Запуск фоновой задачи записи кадров (ядро 1, приоритет 1)
  xTaskCreatePinnedToCore(
      recorderTask,    // функция
      "recorder",      // имя
      4096,            // стек
      NULL,            // параметры
      1,               // приоритет
...
void loop() {
  ...
  if (!sdOK && now - lastSdRetry > 5000) {
      ...
      sdOK = initSD();
      if (sdOK) {
          // SD появилась — ВСЕГДА стартуем запись (безусловно)
          if (!isRecording) {
              isRecording = startRecordingSession();
              if (isRecording) {
                  strncpy(crawlerStatus, "REC_AUTO", sizeof(crawlerStatus) - 1);
