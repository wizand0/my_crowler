...
// ── Автопилот: ≈20% от SPEED_FULL — медленнее для надёжной реакции сонаров
#define SPEED_AUTO        50
#define SPEED_STEER_HI   215
#define SPEED_STEER_LO   120
// Пропорциональное рулевое управление для автопилота
#define SPEED_STEER_AUTO_HI  60
#define SPEED_STEER_AUTO_LO  30
...
void handleAutoFwd() {
  ...
  // Используем SPEED_AUTO (≈20%) — чтобы сонары успевали отрабатывать
  long diff = (long)dR - (long)dL;
  int lSpd = SPEED_AUTO, rSpd = SPEED_AUTO;
  if (diff > HYSTERESIS) {
    lSpd = SPEED_STEER_AUTO_HI; rSpd = SPEED_STEER_AUTO_LO;
  } else if (diff < -HYSTERESIS) {
    lSpd = SPEED_STEER_AUTO_LO; rSpd = SPEED_STEER_AUTO_HI;
  }
  setMotors(lSpd, rSpd);
}
...
void handleAutoTurn() {
  ...
  // Поворот на малой скорости — SPEED_AUTO для контроля
...
void loop() {
  ...
  if (robotState != ST_DESCENT_ALERT && robotState != ST_DESCENT_CRAWL) {
    ...
    // ВАЖНО: hbSeen — не входить в автопилот если ESP32 вообще не выходила на связь
    if (connLost && robotState == ST_MANUAL && hbSeen) {
      robotState = ST_AUTO_FWD;
